...
module wheelMount(radius, height, support_length) {
    ...
    for(a=[00:60:359]) {
        // support
        rotate([0,0,a+30])translate([radius-1,2,2])rotate([90,0,0])linear_extrude(4)polygon([[0,0],[0,height/1.2],[support_length,0]]);
    }
}
...
module pulleyTeeth(teeth, upSideDown = false, addBeltSlot = true) {
    flange = 1.5;
    ...
    difference() {
        union() {
            ...
            pulley(12, teeth);
//            translate([0,-drive_wheel_height/2])square([1,drive_wheel_height]);
        }
        dist = (wheel_radius-4)/1.5;
        for(a=[0:60:300]) {
           rotate ([0,0,a]) {
...
module PulleyRodMount(teeth) {
    ...
    difference() {
        union() {
            pulleyTeeth(teeth, upSideDown = true, addBeltSlot = false);
            wheelMount(radius=19, height=40, support_length=11);
//            // limit switch interruptors
//            for (i=[-30,30,90]) {
//                linear_extrude(drive_wheel_height+8)rotate(i)arc(r=wheel_radius-16+.5, w=1, angle=5, $fa=1);
